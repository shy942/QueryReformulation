...
\begin{figure}
	\centering
	\includegraphics[width=3in]{BGraph2}
...
We evaluate our technique in three different aspects using
three widely used performance metrics and 3,431 bug reports (i.e.,
queries) from four open source subject systems. We compare not only with two baseline techniques but also three state-of-the-art studies from the literature. We also answer three research questions with our experiment as follows: 
\begin{itemize}
	\item \textbf{RQ$_1$}: How does the proposed approach perform in bug localization compared to the baseline approaches? 
	\item \textbf{RQ$_2$}: (a) Can implicit association make any significant difference in IR-based bug localization? (b) Can BLuAMIR overcome the challenges with large documents?   
	\item \textbf{RQ$_3$}: Can the proposed approach outperform the state-of-the-art studies in bug localization?
\end{itemize}
	...
	%\begin{center}
		...
		\begin{tabular}{ l| c | p{2.2cm} | c | c }
		 ...
		 \hline
			...
			Eclipse & v3.1  & Oct 2004 - Mar 2011 & {3071} & 11831\\ \hline
...
\textbf{Recall at Top K / Hit@K:} It represents the percentage of bug reports for each of which at least one ground truth buggy document is successfully retrieved within the Top-K results. Here, $K$ could be 1, 5 or 10.  The higher the Hit@K value is, the better the bug localization performance is.
...
\end{equation*}
where $firstRank(q)$ is the rank of the first buggy source document within the returned result list for each query $q\in Q$. Reciprocal rank takes a value between 0 and 1. Here, 0 value suggests that no buggy document is retrieved whereas 1 value suggests that the first buggy document is retrieved at the topmost position of the result list.
...
\begin{equation*}
AP(q)=\sum_{k=1}^{K}\frac{P(k)\times buggy(k)}{|R|},~~P(k)=\frac{\#buggy ~Docs}{k}
\end{equation*}
where $k$ is a rank in the ranked results, $K$ is the number of total documents, and $S$ is the set of true positive instances. $buggy(k)$ function indicates whether the $k_{th}$ document is buggy file or not. $P(k)$ returns the calculated precision at a given rank position. Since AP is a metric for single query $q$, MAP can be calculated for a set of queries $Q$ as follows:
\begin{equation*}
