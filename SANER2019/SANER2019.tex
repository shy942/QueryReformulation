...
We construct an example bipartite graph for a bug report (ID 322401) from Eclipse UI Platform. Table \ref{tab:BugInfo2} shows the \textit{title} and \textit{description} of the bug report. According to the bug-fix history, these two classes - \texttt{\textbf{S1}:IDEWorkbenchMessages.java} and \texttt{\textbf{S2}: LinkedResourceEditor.java}-- constitute the ground truth for the bug report. Fig. \ref{fig:BipartiteGraph} shows the bipartite graph for the bug report along with its ground truth. We see that ten unique keywords and two source documents represent the two set of nodes and each node is connected to all the nodes from another set. In our approach, we enrich such a graph with nodes and connections generated from each of the bug reports of a subject system.     
...
of our bug localization approach.
We have constructed an association map that connects the keywords from a bug report to its corresponding changed source documents (Section \ref{sec:MapConstruction}). For a given bug report (the query), we return a list of source documents that are not only lexically similar but also strongly associated with the query. Thus, we calculate two scores for each source document and then suggest the Top-K documents as follows:
