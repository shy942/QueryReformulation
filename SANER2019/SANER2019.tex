...
Experiments using a collection of ~3431 bug reports show that on average our technique can locate buggy files with a Top-10 accuracy of 74.06\% and a mean reciprocal rank@10 of 0.52 and a mean precision average@10 of 41\% , which are highly promising. 
Comparison with the state-of-the-art techniques
and their variants report that our technique can improve 32.26\% in
MAP@10 and 26.74\% in Top-5 accuracy over the state-of-the-art.
\end{abstract}
...
\citet{LukinsBL} performed a study on applying LDA (Latent Dirichlet Allocation) to search for bug-related methods and files \cite{LukinsBL}. They used 322 bugs across 25 versions of three projects (Eclipse, Mozilla and Rhino) for the evaluation.
\citet{Nguyen} proposed BugScout \cite{Nguyen}, which customized LDA for bug localization. Results on several large-scale datasets showed good performance.
...
three widely used performance metrics and 3431 bug reports (i.e.,
queries) from four open source subject systems. 
First, we evaluate in terms of the performance metrics, and contrast with two replicated baseline, Latent Semantic Indexing (LSI) \cite{MarcusLSI} and basic Vector Space Model(VSM).
%, BugLocator \cite{Jian} and BLUiR \cite{Saha}.
BLuAMIR localizes bugs with in average 36.98\% higher accuracy (i.e., Hit@10), 63\% higher precision (i.e., MAP@10), and 63.81\% higher result ranks (i.e., MRR@10) than replicated baseline LSI \cite{MarcusLSI} (Section \ref{RQ4answer}).
...
Our technique can localize bugs with in average 58.79\% higher accuracy at Hit@1, 54.01\% higher accuracy at Hit@5 and 49.11\% higher accuracy at Hit@10 than BugScout \cite{Nguyen}.
...
\begin{itemize}
	...
	\item Comprehensive evaluation of the technique using total of 3431 bugs from Eclipse, SWT, AspectJ and Zxing bug repository and validation against 
	%state-of-the-art bug localization technique \cite{Jian}.
	three state-of-the-art approaches including BugScout \cite{Nguyen}, BugLocator \cite{Jian} and BLUiR \cite{Saha}.
\end{itemize}
...
\begin{algorithm}[!t]
	...
	\begin{algorithmic}[1]
		...
		\Comment{$CommitLogs$: a collection of commit messages}
		\State $MAP_{KS} \gets$ \{\}
...
So. to answer our \textit{RQ1}, we can go to Table \ref{tab:Performance2}, 63.30\% bugs are successfully located in Top-10 for Eclipse dataset by BLuAMIR. 
...
\begin{figure}
 ...
 \end{figure}
We compare the performance of our proposed approach in terms of top 1, 5, 10 rank (depicted in Table \ref{tab:Performance2}) and MRR and MAP (shown in Box plot \ref{box:LSI+AssoMRR} and \ref{box:LSI+Asso-MAP} respectively). We can see that our proposed approach outperforms in all case. This also answers our RQ1.
...
For Buglocator \cite{Jian}, we copied the results directly from their paper. We also collect the same bug reports and the same source code repository for both of them. So the results can be compared. 
Our tool BLuAMIR outperforms LSI \cite{MarcusLSI} for both SWT and Zxing dataset in all five performance metrics.
