...
\begin{algorithm}[!t]
	...
	\begin{algorithmic}[1]
		...
		\LineComment{Combine both scores}
		\State $C[d].score\gets VSM[d].score$+$Assoc[d].score$
...
tf(t,d)=\frac{f_{td}}{\#terms},~~idf(t)=log\frac{\#doc}{n_{t}}
\end{equation*}
Here $f_{td}$ refers to the frequency of each unique term {$t$} in a document {$d$}, $n_t$ denotes the document frequency of term $t$ and $\#doc$ is the total number of documents in the collection.   
...
in a document collection. $lexicalSimScore$ takes a value between 0 and 1 where 0 means means total dissimilarity and 1 means strong lexical similarity between the query $Q$ and the candidate source code document $d$. We use Apache Lucene for calculating the lexical similarity.
